(function(){
  ...
  function getRank(score){
    ...
    score.results.forEach((rank)=>{rankArr.push(rank.score)})
    // Sort the scores
    var sorted = rankArr.slice().sort(function(a,b){return b-a})
    // Return ranking results
    var ranks = rankArr.slice().map(function(v){ return sorted.indexOf(v)+1 });
  ...
  function renderExams(exams){
    ...
    let html = ` `;
      exams.exams.forEach(exam =>{
        html =
        `<tr class="getClassInfo" id="${exam.id}">
          <td>Exam ${exam.id}</td><td>${exam.average}</td><td>${exam.studentCount}</td>
        </tr>`
  ...
  function renderSingleExam(id, examInfo){
    ...
    singleExamContainer.style.display = "block"
      ...
      examInfo.results.forEach(exam =>{
        let rows =
        `<tr>
          <td>Student ${exam.studentId}</td><td>${exam.score}</td><td>${exam.rank}</td>
        </tr>`
      ...
      if(studentInfo.length === 0){
        ...
        let html = ` `;
          studentInfo.forEach(student =>{
            html =
            `<tr class="getStudentInfo" id="${student}">
  ...
  function renderStudentExam(name, studentInfo){
    ...
    Object.keys(studentInfo.examResults).forEach((key)=>{
        let rows =
        `<tr>
          <td>Exam (${key})</td><td>Score: ${studentInfo.examResults[key].score} </td>
        </tr>`
        examRow.innerHTML += rows;
    })
